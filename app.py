...
def search_venues():
  ...
  for result in SearchResult:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": len(NewShow.query.filter(NewShow.venue_id == result.id).filter(NewShow.start_time > datetime.now()).all()),
    })
...
def show_venue(venue_id):
  ...
  upcomingShows_query = NewShow.query.join(Artist).filter(NewShow.venue_id==venue_id).filter(NewShow.start_time>datetime.now()).all()
  pastShows_query = NewShow.query.join(Artist).filter(NewShow.venue_id==venue_id).filter(NewShow.start_time<datetime.now()).all()
...
def create_venue_submission():
  ...
  try: 
    ...
    db.session.close()
  if error: 
    flash('An error occurred while create venue')
  if not error: 
...
def search_artists():
  ...
  for result in search_result:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": len(NewShow.query.filter(NewShow.artist_id == result.id).filter(NewShow.start_time > datetime.now()).all()),
    })
...
def show_artist(artist_id):
  ...
  pastShows_query = NewShow.query.join(Venue).filter(NewShow.artist_id==artist_id).filter(NewShow.start_time>datetime.now()).all()
  ...
  upcomingShows_query = NewShow.query.join(Venue).filter(NewShow.artist_id==artist_id).filter(NewShow.start_time>datetime.now()).all()
...
def create_artist_submission():
  ...
  try: 
    ...
    db.session.close()
  if error: 
    flash('An error occurred. Artist ' + request.form['name']+ ' not  listed.')
  if not error: 
...
def shows():
  ...
  # TODO: replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  showsQuery = NewShow.query.join(Artist).join(Venue).all()
...
def create_show_submission():
  ...
  try: 
    ...
    show = NewShow(artist_id=artist_id, venue_id=venue_id, start_time=start_time)
    db.session.add(show)
    db.session.commit()
  except: 
    error = True
